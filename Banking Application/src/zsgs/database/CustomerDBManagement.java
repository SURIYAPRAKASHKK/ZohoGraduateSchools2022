...
public class CustomerDBManagement extends JDBC_Connection {
	...
	//private static final String FETCH_ALL_CUSTOMERS = "SELECT AccNo FROM customer_details";
	...
	public Boolean accExist(short accNo) {
	...
	/*public Boolean upiCheck(String upiId, String password) {

		connection = connector();
		int count = 0;
		String upiId1 = null, password1 = null;
		try {
			preparedStatement = connection.prepareStatement(UPI_CHECK);
			preparedStatement.setString(1, upiId);
			resultset = preparedStatement.executeQuery();
			while (resultset.next()) {
				upiId1 = resultset.getString("UPI_ID");
				password1 = resultset.getString("Password");
			}
			if(upiId1.equals(upiId) && password1.equals(password)) {
				count = 1;
			}
		} catch (SQLException e) {
	...
	public Boolean upiExist(String upiId) {
	...
	public ArrayList<User> fetchUserDetail(){
		ArrayList<User> allCustomersList = new ArrayList<>();
		connection = connector();
		try {
			ResultSet results;
			preparedStatement = connection.prepareStatement("Select AccNo From Customer_Details Order by AccNo");
			results = preparedStatement.executeQuery();
			while(results.next()) {
				user = fetchUserDetail(results.getShort(1),1);
				allCustomersList.add(user);
			}
		} catch (SQLException e) {
			System.err.println("Unable to Fetch All Account No.");
		}
